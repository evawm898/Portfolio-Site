...
### The census list: 233 -> 211, and TWENTY of the twenty-two removals were main's own

Generated by the magnitude tool's own `--emit` (the fast 72) and
`--emit --include-refused` (the seven `ALL MAX` rows) and applied
mechanically: **57 entries re-recorded, 22 removed, 233 -> 211.**

**THE CLAMP FIXES PRE-EXISTING FOLDS RATHER THAN TIDYING UP AFTER THE NIB.**
Of the 22 entries that came off, **20 were declared on main before this PR**,
with main's own figures:

| main | row |
|---|---|
| 80 / 0.0239 | `petalCup max (1.2)` |
| 94 / 0.1546 | `VARIANCE: x cup 1.2 (the size field composed with the form maximum)` |
| 297 / 0.2054 | `6 layers x innerCup min (-0.8)` |
| 160 / 0.1264 | `ZYGO: cup clamp binds (base 1.2 + delta 1.2 -> 1.2)` |
| 20 / 0.1264 | `SLOT: labellumCup max (1.2) x 2 whorls in step` |
| 20 / 0.1264 | `SLOT: hoodCup max (1.2) x 2 whorls in step` |
| 10-20 / 0.0239 | `PER-PETAL: petal1Cup max` ... `petal9Cup max` (all nine) |
| 160 / 0.1371 | `GRADIENT: cup gradient max x cup max x widest petal` |
| 248 / 0.2240 | `GRADIENT: cup gradient min x cup min (reflexed, more so at the tip)` |
| 48 / 0.1612 | `BUCKLE: over a cupped blade (cup 1.2)` |
| 115 / 0.0953 | `SEPALS: sepalCup max (1.2)` |
| 115 / 0.1089 | `SEPALS: sepalCupGradient max (1.2)` |

The other **two** are this PR's own — `ORCHID: the labellum and the hood x 2
whorls in step` and `FAN x PER-PETAL: petal 1 extreme x toggle OFF` — added by
the nib and now fixed by the clamp.

`ALL MAX` moves **107,485 -> 91,808 pairs at an unmoved 10.1332 mm**; its
`EXPORT_REFUSED_XFAIL` entry is untouched at 3,090,816 triangles.

**THE SELF-CHECK: 0 STALE, 0 UNDECLARED.** Run over the 117 movers, which is
equivalent to the whole matrix and not a sample — the byte partition proves
**0 floats moved on the 803 holders**, so a holder's census cannot have
changed. 117 of 117 read their record or read clean.

**AND THREE TRAPS ON THE WAY, EACH CAUGHT BY A CHECK RATHER THAN BY READING**:
the FAIL findings printed after the emit block share its `  "label":` shape,
so an unbounded parse replaced every entry with its own finding text (caught
by `node --check`, which a dry run cannot do because a dry run writes
nothing); `ALL MAX` is a key of `EXPORT_REFUSED_XFAIL` too, so an unscoped
edit rewrote the REFUSAL entry — caught at module load by #213's own *"a
declaration without a number is a label"*; and a character scanner over this
block is fooled by the apostrophes in its own header comments, which is the
retired-id scanner's lesson arriving from the other side. The applier is
line-wise.
