...
for (const c of CONFIGS) {
  ...
  // The replica is what every option column is measured on. If it has drifted from the
  // shipped buildVoronoi, this table describes geometry that does not ship — which is
  // exactly what happened when #77 moved Voronoi's clip bound and this harness kept
  // reading the untrimmed one. Fail loudly; do not emit a plausible-looking table.
  if (r.replica && !r.replica.valid) {
    console.error(`REPLICA DRIFT: ${c.name} — replica ${r.replica.replicaCells} cells / ${r.replica.replicaArea.toFixed(4)} vs real ${r.replica.realSlabs} / ${r.replica.realOuterArea.toFixed(4)} (cellGap ${r.replica.cellGap}, areaGap ${r.replica.areaGap}). The replica no longer matches the shipped builder.`);
    process.exitCode = 1;
  }
