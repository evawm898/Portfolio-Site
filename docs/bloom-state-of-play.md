...
| 13 | **`tools/bloom-wall-thickness.mjs` STILL CARRIES NO INFILLED STATE.** V5 is no longer the only gate on self-approach — #263's CG2 is the second, on a product grid — but neither carries an infilled state, and the combination gate is the natural home for one (a pair, or a `base` set on an existing pair). Recorded by #250, measured around by #252, measured around again by the lamina-floor session. | `docs/bloom-infill-lamina-floor.md:469-472`; `docs/bloom-infill-base-panel.md:493-497` | The infill shipping. It must gain one the day the infill lands. |
...
| 20 | **GATED (#263), and it is a MEASUREMENT gap rather than a combination one.** A leaf blade approaches the free stem under `MIN_FEATURE_MM` from `leafAngle` 70° up — 0.824 / 0.289 / 0.000 mm at 70 / 75 / 80–90, reproduced exactly by `tools/bloom-combination-gate.mjs` through an instrument sharing no code with the one that produced it. Swept over six partners the SECOND control moves it by at most **0.002 mm**, so the hazard is `leafAngle`'s alone and `LEAVES: the STEEP angle (85 deg)` was already a matrix row — nothing measured blade-to-stem approach at all. The pair ships `verdict: single-reaches` so CG4 asserts that. Eva's ruled 35° reads 4.019 mm. **#265 added the seventh partner and it is the strongest reading yet: `leafToothDepth` moves the approach by EXACTLY 0.000e+0 at all four angles — bit-identical, not merely under the band — because the teeth are cut into the leaf's MARGIN and the nearest blade point to the stem is at its BASE.** That pair ships in tier 3 by ruling, with the inertness declared in `COMBINATION_INERT` and held in both directions by CG7. | `docs/bloom-combination-gate.md` §4; `docs/bloom-leaves-outcome.md`; `CLAUDE.md` leaves block | Nothing. Whether the top fifth of the angle range is accepted or bounded is Eva's ruling. |
| 21 | **GATED (#263).** `cup × petalTipShape` self-approaches below the 1.00 mm printable gap — 1.031 mm at tip shape 1.20, 0.977 at 2.50, 0.832 at 3.00 on `SHIPPED cup 1.2`, reproduced EXACTLY by `tools/bloom-combination-gate.mjs` and declared as magnitudes there. **The matrix varies ONE CONTROL AT A TIME, so this was invisible to it by construction.** **ALL FIFTEEN BOUGHT PAIRS SHIP (Eva's ruling, #265), taking the gate to twenty: 5 tier-1 (#263's) · 9 tier-2 · 6 tier-3, 262 cells, 78 declared cells across sixteen pairs, in `bloom-export-watertight.yml`.** The gate found the hazard reaches DOWN to cup 0.6 at tip shape 3.00 (0.929 mm) and six failing cells on `cup × curl` from 0.945 to **0.012 mm**; #265 added `petalSpineCurl × petalTwist` at **0.012 mm with BOTH singles clear** and `petalCup × petalRoll`'s eleven cells down to **0.010**. **`verdict` is three-valued now — four of the fifteen pairs CLEAR, which #263's boolean had no true value for — and CG6 (provenance) and CG7 (the inert record) are the other two clauses the expansion forced.** | `docs/bloom-combination-gate.md`; `docs/bloom-session-32-outcome.md` §18a; `CLAUDE.md` | Nothing now. It is the instrument inflorescence ruling 11 requires and the one bell/corolla question 5 asks for. Whether the hazard is accepted as a look or fixed is still Eva's ruling. |
...
| **21 — the combination gate (BUILT, #263; all fifteen pairs, #265)** | `tools/bloom-combination-gate.mjs` · `tools/bloom-wall-thickness.mjs` (it EXPORTS `measureWall`, which the gate imports) · `tools/bloom-xfail-magnitudes.mjs` (`--combination`) · `.github/workflows/bloom-export-watertight.yml` · NOT `tools/bloom-harness.mjs` (the gate deliberately does not import it, which is what lets it run before the npm install) | **1 (ruling 11's prerequisite, now satisfied), 5 (bell question 5, now answerable from numbers), 13, 20, 26** |
