...
// Derived from THIS FILE's location, never hardcoded. The gate both serves the
// page and resolves node_modules from ROOT, so a hardcoded absolute path makes
// a run from a git worktree (which is how a before/after against another
// commit is done here) silently measure the checkout it was NOT pointed at,
// and report the result as though it were the worktree's.
...
async function run({ mutant = null, shots = false } = {}) {
  ...
  out(`new bundle still posable: bends=${newBundleBends} clampsToOwnLimits=${newBundleClampsToOwnLimits} orbits=${newBundleOrbits}`);
  check('swap/file-input', inputSwapOK, `source=${afterInput.source} limits=${JSON.stringify(afterInput.limits)}`);
  check('swap/still-posable', newBundleBends, `handle over canvas=${handle2OnCanvas}`);
  check('swap/clamps-to-own-limits', newBundleClampsToOwnLimits, `droop/twist after out-of-range forcePose`);
  check('swap/still-orbits', newBundleOrbits);
  ...
  check('swap/drop', dropSwapOK, `hint idle=${hintHiddenIdle} dragging=${hintShownWhileDragging} after=${hintHiddenAfterDrop}`);
  ...
  check('swap/invalid-file-fails-visibly', invalidFileOK, `lastLoadError="${lastLoadErrorBad}"`);
  ...
  check('swap/no-pivot-fails-visibly', noPivotOK, `extras=${JSON.stringify(noPivotExtras)} poseHidden=${noPivotPoseHidden}`);
  ...
  check('swap/recovers-after-bad-load', recoveryOK, JSON.stringify(recovered));
  ...
  // --- the line art ACROSS a swap ------------------------------------------
  // These four claims exist only because the runtime loader and the line-art
  // stage now ship together. `art`'s draw objects are parented to the BUNDLE's
  // meshes, so a swap has to tear the old one down and build a new one, and
  // the STYLIZE controls have to go on driving whatever `art` currently is
  // without being re-bound. None of that is visible to any check above: a
  // build that stacked a second set of listeners, or left the previous
  // bundle's objects in the scene, still swaps, still poses and still draws.
  const artAfterSwap = await page.evaluate(() => ({
    built: window.__printLineArt.built(),
    objects: window.__printLineArt.lineArtObjectsInScene(),
    topo: window.__printLineArt.topology(),
    style: window.__printLineArt.style(),
    enabled: window.__printLineArt.enabled(),
  }));
  out(`line art after swaps: built=${artAfterSwap.built} sceneObjects=${artAfterSwap.objects} meshes=${artAfterSwap.topo.map(t => t.name).join('+')}`);

  check('swap/lineart-rebuilt',
    artAfterSwap.built && artAfterSwap.topo.length > 0
      && artAfterSwap.topo.every(t => t.tris > 0 && t.edges > 0),
    artAfterSwap.topo.map(t => `${t.name}:${t.tris}tri/${t.edges}e`).join(' '));

  // Four draw objects per source mesh (contour+interior x lines+dots). More
  // than that means a previous bundle's art was never disposed.
  check('swap/lineart-no-stale-objects',
    artAfterSwap.objects === artAfterSwap.topo.length * 4,
    `${artAfterSwap.objects} in scene, expected ${artAfterSwap.topo.length * 4} for ${artAfterSwap.topo.length} mesh(es)`);

  // The panel is not reset by a swap: whatever the sliders say is pushed onto
  // the newly built art, so the style outlives the bundle it was set on.
  await call(() => window.__printLineArt.setWeightWidget(4.1));
  await call(() => window.__printLineArt.setDetailWidget(63));
  const preSwapStyle = await call(() => window.__printLineArt.style());
  const againBuf = readFileSync(path.join(ROOT, 'assets/print-test/flower-test-bundle.glb'));
  await page.setInputFiles('#bundleFile', { name: 'restyled-bundle.glb', mimeType: 'model/gltf-binary', buffer: againBuf });
  await page.waitForFunction(() => window.__printScaffold && window.__printScaffold.source === 'restyled-bundle.glb', { timeout: 15000 });
  const postSwapStyle = await call(() => window.__printLineArt.style());
  const postSwapObjects = await call(() => window.__printLineArt.lineArtObjectsInScene());
  check('swap/lineart-style-survives',
    JSON.stringify(preSwapStyle) === JSON.stringify(postSwapStyle),
    `${JSON.stringify(preSwapStyle)} vs ${JSON.stringify(postSwapStyle)}`);
  check('swap/lineart-no-accumulation-over-swaps',
    postSwapObjects === artAfterSwap.objects,
    `${artAfterSwap.objects} -> ${postSwapObjects} after another swap`);

  // And it must still be LIVE on the new bundle: bending the swapped-in stem
  // has to change its extracted segment count, which only happens if repose()
  // is refreshing the geometry of the CURRENT stem mesh rather than the one
  // the page happened to load first.
  await call(() => window.__printLineArt.setLineArt(true));
  // The swap RESET the pose, so the stem is at rest right now — comparing
  // rest against rest would read as "unchanged" no matter what refreshGeometry
  // does. Bend it first, through the canvas, so there is a difference to see.
  const [lhx, lhy] = await call(() => window.__printScaffold.handleScreenPos(2));
  await page.mouse.move(lhx, lhy); await page.mouse.down();
  for (let i = 1; i <= 10; i++) await page.mouse.move(lhx + i * 6, lhy);
  await page.mouse.up(); await page.waitForTimeout(200);
  const bentForLive = !(await call(() => window.__printScaffold.isRest()));
  const liveAcross = await call(() => window.__printLineArt.stemLinesRestVsBent());
  out(`line art live on swapped bundle: ${JSON.stringify(liveAcross)}`);
  check('swap/lineart-live-on-new-bundle',
    bentForLive && !!liveAcross && liveAcross.bent !== liveAcross.rest
      && liveAcross.restored === liveAcross.bent,
    liveAcross ? `bent=${bentForLive} ${liveAcross.bent} vs rest ${liveAcross.rest}, restored ${liveAcross.restored}` : 'null');
