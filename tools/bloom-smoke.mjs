  ...
  },
  {
    ...
    rows: [
      ...
      { label: 'APEX: a ROUNDED TRUNCATE — the poppy, and the state no shipped control could reach',
        path: 'both apex controls off their identities at once: A6 (the cap built at the exponent the effective state asks for — the only witness for a shape silently held at 1, which passes A1-A5 on every row) and A5 (the apex narrows monotonically — the retired TIP_PLATEAU\'s waist, which both STL gates are blind to, and the only witness for a cap that widens in the emitted rows)' },
