  ...
  },
  {
    ...
    rows: [
      ...
      { label: 'APEX: broad end (max) — the cap now converges to it, it does not square off',
        path: 'the unconditional cap at its widest terminal: A2 (the last emitted row IS the terminus), A3 (never a true apex) and A4 (the terminal rebuilt from the state and the mode floor) all read a NON-default terminal here, which is what makes this the row that would catch a cap that only works at the identity' },
      { label: 'APEX: a ROUNDED TRUNCATE — the poppy, and the state no shipped control could reach',
        path: 'both apex controls off their identities at once: A6 (the cap built at the exponent the effective state asks for — the only witness for a shape silently held at 1, which passes A2, A3, A4 and A5 on every row) and A5 (the apex narrows monotonically — the retired TIP_PLATEAU\'s waist, which both STL gates are blind to, and the only witness for a cap that widens in the emitted rows)' },
