...
async function shoot(name, rows, viewport, note){
...
cells.push(await shoot('01-unrated-desktop', build(false), { width:1280, height:1000 },
  'As the library looks the day this ships: nothing rated, nothing marked. The marks column is empty and the row is what it was.'));
cells.push(await shoot('02-rated-desktop', build(true), { width:1280, height:1000 },
  'After a triage pass — the state nobody can see by opening the page today, and the one the question is actually about.'));
cells.push(await shoot('03-rated-mobile', build(true), { width:390, height:900 },
  'On a phone, where .entry-index is hidden and the stars are NOT a target at all (pointer-events, behind hover:hover).'));
cells.push(await shoot('04-unrated-mobile', build(false), { width:390, height:900 },
  'The same phone view with nothing set.'));
