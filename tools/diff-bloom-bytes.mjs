/* ===================================================================
   ...
   bytes the real Get STL button produced.
     ...
     --phase3   `phase3Matrix()` — the 86 rows frozen at 6626961, the commit
                ...
                claims on its own.
     --phase21  `phase21Matrix()` — the 572 rows frozen at b323268, the head
                of main before session 32's retirement took the live matrix to
                562. THE NEWEST BASELINE, and the newest one that still
                exercises `petalTipBreadth` and its three role twins. Like
                phase19 and phase20 it names retired ids, so a plain re-export
                from a later tree refuses those rows by design and the close
                is the three-capture construction.
     ...
     --phase18  `phase18Matrix()` — the 528 rows frozen at cb798f6, the head
                ...
                (session 26) with blocks 23 and 24 both present. A plain
                capture per tree and --compare close it.
     --phase17  `phase17Matrix()` — the 507 rows frozen at 6335ac4, the head
                of main when session 22 opened. The like-for-like baseline
                for the gynoecium, and the first to carry the androecium's
                own block-23 rows — the witness for the rod/pill helpers'
                verbatim extraction. NOTE (session 24): eight of its rows no
                longer reproduce their bytes — the Vogel disc's inner limit
                moved them, by ruling; its row DEFINITIONS are unchanged and
                --verify-frozen still proves them.
   ...
   RUN:  node tools/diff-bloom-bytes.mjs [--full|--phase2..--phase21] --root <dir> --out <file.json> [--only <regex>] [--override <id>=<value>[,<id>=<value>…]] [--strip <id,id,...>]
         ... twice, then:
...
/* THE MAP MOVED TO THE HARNESS (session 32) and is imported, not restated.
   It is the same list this file has always carried; what changed is that the
   harness now asserts at module load that it agrees with FROZEN_BASE_COMMITS,
   which is the check that was missing when phase21 shipped registered here
   and nowhere else — verified by this file, pinned by no tag, and iterated by
   neither CI loop. A second copy here would put that census back to guessing. */
const FROZEN = FROZEN_MATRICES;
