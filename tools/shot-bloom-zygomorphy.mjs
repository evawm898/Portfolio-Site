...
extremes.push(await cell({
  ...
  set: set({ layerCount: 3, innerCurl: 360, innerCup: 1.2, layerTilt: 20 }),
...
extremes.push(await cell({
  ...
  set: set({ layerCount: 3, petalCount: 40, innerCurl: 360, innerCup: 1.2, sheetThickness: 0.6, tipThinning: 0.8, footDelicacy: 0.25, spread: 0.6 }),
...
extremes.push(await cell({
  ...
  set: set({ placement: 'CONTINUOUS', layerCount: 3, innerCurl: 360, innerCup: 1.2 }),
...
for (const [name, title, note, cells, perRow] of [
  ...
  ['zygomorphy-deltas', 'Zygomorphy — the three deltas, swept one at a time',
   'EACH DELTA ALONE, against the same control at the left, so what each control does is separable rather than inferred from a composite (the form sheet\'s discipline). Curl reads in PROFILE and nowhere else; cup reads FACE-ON. THE TIP CELL IS GONE: session 32 retired the tip deltas with `petalTipBreadth`, so the apex is one unconditional cap with no control and no role can differentiate it. It returns when the superellipse law does. SIZE AND TILT ARE ABSENT ON PURPOSE: `layerSize` and `layerTilt` are λ-ramps that already own per-layer size and tilt, and a role override of either would be a second owner of one quantity — and would strand CONTINUOUS, whose only depth controls those are.',
   pairs(singles), 5],
