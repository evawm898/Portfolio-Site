...
for (const [label, s, note] of [
  ...
  ['inner tip +0.60 (standards blunt, falls pointed)', { innerTipEnd: 0.6 }, 'TWO DIFFERENT APEX TERMINALS IN ONE BLOOM, and it is a state no single-whorl design can reach. Since Eva\'s `blunt` ruling (session 32) there is ONE apex construction rather than two: the converging cap is unconditional and `petalTipEnd` is its terminal half-width, so the outer whorl runs to the mesh floor and the inner whorl to 60% of its width, both by the same monotone law. The EXACT `=== 0` branch this cell used to photograph is retired with `petalTipBreadth`. FACE-ON is the view.'],
...
extremes.push(await cell({
  ...
  set: set({ layerCount: 3, innerCurl: 360, innerCup: 1.2, innerTipEnd: 0.6, layerTilt: 20 }),
...
extremes.push(await cell({
  ...
  set: set({ layerCount: 3, petalCount: 40, innerCurl: 360, innerCup: 1.2, innerTipEnd: 0.6, sheetThickness: 0.6, tipThinning: 0.8, footDelicacy: 0.25, spread: 0.6 }),
...
extremes.push(await cell({
  ...
  set: set({ placement: 'CONTINUOUS', layerCount: 3, innerCurl: 360, innerCup: 1.2, innerTipEnd: 0.6 }),
...
for (const [name, title, note, cells, perRow] of [
  ...
  ['zygomorphy-deltas', 'Zygomorphy — the three deltas, swept one at a time',
   'EACH DELTA ALONE, against the same control at the left, so what each control does is separable rather than inferred from a composite (the form sheet\'s discipline). Curl reads in PROFILE and nowhere else; cup and the tip terminal read FACE-ON. The tip cell is the subtle one: a positive delta on the inner whorl gives the two whorls DIFFERENT APEX TERMINALS in one export — one construction, two ends — which no single-whorl design can reach. SIZE AND TILT ARE ABSENT ON PURPOSE: `layerSize` and `layerTilt` are λ-ramps that already own per-layer size and tilt, and a role override of either would be a second owner of one quantity — and would strand CONTINUOUS, whose only depth controls those are.',
   pairs(singles), 5],
