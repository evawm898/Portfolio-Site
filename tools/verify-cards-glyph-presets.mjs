 ...
 * the three families are actually distinguishable from each other and from
...
check(GLYPH_FAMILIES.length === 3, `3 built-in families declared (${GLYPH_FAMILIES.map((f) => f.id).join(', ')})`);
...
// calibrated against MEASURED values for three real, distinct hand-authored
// shapes (0.380–0.919 pairwise between the three families) rather than
// picked to make a first run pass; this is the bar "the three families are
...
for (const family of GLYPH_FAMILIES) {
  ...
  for (const [otherLabel, otherMask] of Object.entries(familyMasks)) {
    ...
    check(d > DIFFERENT, `"${family.label}" reads as a different SHAPE from "${otherLabel}" (${d.toFixed(3)} > ${DIFFERENT}) — the three families are not near-duplicates`);
  }
...
// ---------------------------------------------------------------------
await clickFamily('classic'); // a bulbous family whose bounding box is easy to measure
