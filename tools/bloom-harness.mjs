...
export const { ROLL_MIN_RADIUS_FACTOR, SHEET_THICKNESS_MM, MIN_FEATURE_MM, FOOT_MIN_WIDTH_MM, FOOT_MAX_WIDTH_MM, MAX_LAYERS, GOLDEN_ANGLE, SPIRAL_LEGIBLE_COUNT,
         ...
         MAX_FAN_GROUPS, PETAL_ROLE_ORDER, petalGroupCount, perPetalEligible, ROLE_ALL, allPetalsEligible } = await import(pathToFileURL(path.join(ROOT, 'bloom-geometry.js')).href);
...
export async function junctionAssertions(page, row) {
  ...
  if (!Array.isArray(roots) || roots.length !== layers.length) {
    ...
    layers.forEach((r, L) => {
      ...
      const wantAngle = t + (q.curlRad * q.u) / 2;
      ...
      /* (b) THE ROOT NORMAL, flat-form rows only — exact, and the stronger
