/* ===================================================================
   shot-bloom-cup-nib.mjs — THE CUP AT THE NIB, LOOKED AT
...
const NOFIX = argOf('--nofix');
if (!NOFIX) { console.error('usage: node tools/shot-bloom-cup-nib.mjs <dir> --nofix <worktree>'); process.exit(2); }
...
const ROWS = [
  { key: 'C', title: 'Row C — 35 x 16, tip shape 0.60, tip taper 1.80, cup +0.94',
    label: 'C (35x16, tipShape 0.60, taper 1.80, cup +0.94)',
    /* THE MATRIX'S OWN SET SHAPE — a list of {id, value}, which is what
       `applyConfig` walks. 35 x 16 and tip taper 1.80 ARE the registry
       defaults, so what this row really varies is the tip shape and the cup;
       they are written out anyway because Eva named them. */
    set: [{ id: 'petalLength', value: '35' }, { id: 'petalWidth', value: '16' },
          { id: 'petalTipShape', value: '0.6' }, { id: 'petalTipTaper', value: '1.8' },
          { id: 'petalCup', value: '0.94' }],
    note: 'The acute apex under a strong positive cup — the nib is at its longest here, so the radius falls furthest.' },
  { key: 'cupmin', title: 'petalCup min (-0.8)', label: rowOf('petalCup min (-0.8)').label,
    set: rowOf('petalCup min (-0.8)').set,
    note: 'A matrix row, undeclared on the branch and exactly 0 on main. The cup at its negative extreme.' },
  { key: 'sepalcup', title: 'SEPALS: sepalCup min (-0.8)', label: rowOf('SEPALS: sepalCup min (-0.8)').label,
    set: rowOf('SEPALS: sepalCup min (-0.8)').set,
    note: 'The sepal is the petal builder on a second ring, so it gets its own nib and its own cup — and being scaled down it meets the floor sooner.' },
];
const TREES = [
  { key: 'nofix', root: NOFIX, label: 'current branch (no cup fix)' },
  { key: 'fix', root: HERE, label: 'cup scaled by h / TIP_HALF_MM on the nib' },
];
...
async function measure(page) {
  return page.evaluate(() => {
    ...
    return { tris: m.exportTris ?? m.liveTris ?? null,
...
async function cell({ page, port, row, file, frame }) {
  ...
  const notes = [];
  for (const view of ['nib', 'sect', 'tip']) {
  ...
  for (const view of ['sect', 'nib', 'tip']) {
    ...
    for (const tree of TREES) {
      ...
      html += `<td><img src="${row.key}-${tree.key}-${view}.png"><div class=cap><b>${esc(tree.label)}</b> &middot; ${view}<br>
        ...
        nib from u ${m.nibFromU?.toFixed(6)} &middot; ${m.onNibRows} rows on it
