...
export class LineArt {
  ...
  // Undoes the constructor. The draw objects are parented to the SOURCE
  // meshes, so a page that swaps bundles has to call this before disposing
  // the outgoing bundle: the meshes' original materials are swapped out for
  // the paper fill while line art is on, and would otherwise be unreachable
  // from the scene tree and never freed.
  dispose() {
    for (const u of this.units) {
      u.mesh.material = u.original;          // restore before anything is freed
      for (const t of u.tiers) {
        u.mesh.remove(t.lines); u.mesh.remove(t.dots);
        t.geo.dispose(); t.mat.dispose();
        t.dotGeo.dispose(); t.dotMat.dispose();
      }
      u.fill.dispose();
    }
